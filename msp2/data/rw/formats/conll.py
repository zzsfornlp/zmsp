...
from msp2.utils import Registrable, Conf, zwarn
from msp2.data.inst import DataInstance, Doc, Sent, Frame, ArgLink
...
# --
# @DataFormator.reg_decorator("??"), note: no default one!!
class ConllFormator(DataFormator):
    ...
    def to_obj(self, inst: DataInstance) -> str:
        ...
        if conf.f_pred is not None:  # frames
            ...
            ret_preds, ret_pred_ids, ret_frames = ConllHelper.put_preds(sent.events, slen)
        ...
        all_fields = [["-"]*num_field for _ in range(slen)]
        for k in all_keys:
            vs = all_cols[k]
            for i in range(slen):
                all_fields[i][k] = vs[i]
    ...
    def from_obj(self, s: str) -> DataInstance:
        ...
        if conf.f_pred is not None:  # frames
            ...
            new_frames = [sent.make_event(p_widx, 1, type=p_lab) for p_widx, p_lab in preds]  # note: wlen==1
            # args?
            if conf.f_arg_start is not None:
                f_arg_start = int(conf.f_arg_start)
                # --
                ...
                for one_new_frame in new_frames:
                    ...
                    for a_widx, a_wlen, a_lab in args:
                        new_ef = sent.make_entity_filler(a_widx, a_wlen, type="UNK")
...
# =====
# special one for srl
@DataFormator.reg_decorator("srl_he_bio")
class HSRLFormator(DataFormator):
    def __init__(self):
        pass

    def to_obj(self, inst: DataInstance) -> str:
        lines = []
        for evt in sorted(inst.events, key=lambda x: x.mention.widx):
            widx, wlen = evt.mention.get_span()
            assert wlen == 1
            words = evt.sent.seq_word.vals
            labels = ["O"] * len(words)
            for arg in evt.args:
                role = arg.role
                a_widx, a_wlen = arg.arg.mention.get_span()
                a_labs = ["B-" + role] + ["I-" + role] * (a_wlen - 1)
                assert all(z=="O" for z in labels[a_widx:a_widx+a_wlen])
                labels[a_widx:a_widx+a_wlen] = a_labs
            lines.append(f"{widx} {' '.join(words)} ||| {' '.join(labels)}")
        if len(lines) == 0:
            return ""
        else:
            return "\n".join(lines) + "\n"
...
# shortcuts
DataFormator.reg("conll05o", lambda: ConllFormator(  # old version
    None, f_word=0, f_xpos=1, f_parse=2, f_pred=5, f_pred_id=4, f_arg_start=6))
DataFormator.reg("conll05", lambda: ConllFormator(  # note: add doc info!!
    None, f_doc=0, f_word=1, f_xpos=2, f_parse=3, f_pred=6, f_pred_id=5, f_arg_start=7, f_ne=4))
DataFormator.reg("conll12", lambda: ConllFormator(
...
DataFormator.reg("conllpb", lambda: ConllFormator(
    None, f_doc=0, f_part=1, f_widx=2, f_word=3, f_xpos=4, f_parse=5, f_pred=6, f_pred_id=7, f_arg_start=8))
DataFormator.reg("conllu", lambda: ConllFormator(
    None, f_widx=0, f_word=1, f_upos=3, f_xpos=4, f_dep_head=6, f_dep_label=7, sep_in='\t', widx_start=1))
DataFormator.reg("conllup", lambda: ConllFormator(  # up
