...
class SeqVocab(SimpleVocab):
    def __init__(self, base_vocab: SimpleVocab, conf: SeqVocabConf=None, **kwargs):
    ...
    def get_allowed_transitions(self):
        ...
        full_size = len(self)
        ret_mat = np.full((full_size, full_size), 0., dtype=np.float32)  # todo(note): others are all 0.
    ...
    def spans2tags_str(self, spans: List[Tuple], length: int): return self.helper_str.spans2tags(spans, length)
    def tags2spans_str(self, tags: List): return self.helper_str.tags2spans(tags)

    def spans2tags_idx(self, spans: List[Tuple], length: int):
        offset = self.base_vocab_offset
        spans0 = [(z[0], z[1], z[2]-offset) for z in spans]
        return self.helper_idx.spans2tags(spans0, length)
...
class SeqSchemeHelper:
    def __init__(self, scheme: str, full_bies: Iterable, tag_o: object):
        ...
        self.scheme = scheme
        self.output_bies = {"BIO": [full_bies[z] for z in [0,1,1,0]], "BIOES": full_bies}[scheme]  # for output
        self.parse_bies = {"BIO": [full_bies[z] for z in [0,1]]+[None,None], "BIOES": full_bies}[scheme]  # for input
        self.num_mul = {"BIO": 2, "BIOES": 4}[scheme]
        self.tag_o = tag_o
    ...
    # note: return Layers[Sequence[...]]
    def spans2tags(self, spans: List[Tuple], length: int) -> List[Tuple[List, List]]:
        t_o = self.get_tag_o()
