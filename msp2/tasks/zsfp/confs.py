...
from msp2.proc import SVConf, FrameEvalConf, MyFNEvalConf, MyPBEvalConf, TRConf
...
class DConf(Conf):
    def __init__(self):
        ...
        self.eval_conf = ConfEntryChoices({'frame': FrameEvalConf(), 'fn': MyFNEvalConf(), 'pb': MyPBEvalConf()}, 'frame')
        # pretrain
...
class OverallConf(Conf):
    def __init__(self):
        ...
        self.mconf = ConfEntryChoices({"basic": ZsfpModelConf(), "mtl": MtlModelConf()}, 'basic')

# get model
def build_model(conf: OverallConf, *args, **kwargs):
    from .model import ZsfpModelConf, ZsfpModel
    from .model2 import MtlModelConf, MtlModel
    mconf = conf.mconf
    if isinstance(mconf, ZsfpModelConf):
        model = ZsfpModel(mconf, *args, **kwargs)
    elif isinstance(mconf, MtlModelConf):
        model = MtlModel(mconf, *args, **kwargs)
    else:
