...
def zlog(s, func="plain", end='\n', flush=True, timed=False):
    Logger._instance._log(str(s), func, end, flush, timed)
...
class Logger(object):
    ...
    _logger_heads = {
        ...
        "debug":"DE ", "none":"**INVALID-CODE**", "nope": "",
    }
    @staticmethod
    ...
    def _log(self, s, func, end, flush, timed):
        func, head = Logger._get_ch(func)
        if self.func_filters[func]:
            if timed:
                ss = head + "[" + time.ctime() + "]" + s
            else:
                ss = head + s
            for f in self.fds:
